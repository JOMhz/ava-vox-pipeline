...
class FastTrainLoader(object):
    ...
    def __getitem__(self, index):
        ...
        audioSet = generate_audio_set(self.audio_path, batchList) # load the audios in this batch to do augmentation
        for line in batchList:
            data = line.split('\t')            
            audioFeatures.append(load_audio(data, self.audio_path, num_frames, audio_aug = True, audioSet = audioSet))  
            visualFeatures.append(load_visual(data, self.visual_path, num_frames, visual_aug = True))
            labels.append(load_label(data, num_frames))
        return torch.FloatTensor(np.array(audioFeatures)), \
               torch.FloatTensor(np.array(visualFeatures)), \
...
class FastValLoader(object):
    def __init__(self, trial_file_name, audio_path, visual_path, current_fold_video_ids=None, **kwargs):
        ...
        self.miniBatch = mixLst

    def __getitem__(self, index):
        line       = [self.miniBatch[index]]
        num_frames  = int(line[0].split('\t')[1])
        audioSet   = generate_audio_set(self.audio_path, line)        
        data = line[0].split('\t')
        video_id = data[0].rsplit('_', 2)[0]
        audioFeatures = [load_audio(data, self.audio_path, num_frames, audio_aug = False, audioSet = audioSet)]
        visualFeatures = [load_visual(data, self.visual_path, num_frames, visual_aug = False)]
        labels = [load_label(data, num_frames)]         
        return video_id, torch.FloatTensor(np.array(audioFeatures)), \
               torch.FloatTensor(np.array(visualFeatures)), \
               torch.LongTensor(np.array(labels))
