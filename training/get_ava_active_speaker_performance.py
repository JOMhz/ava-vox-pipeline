...
def merge_groundtruth_and_predictions(df_groundtruth, df_predictions):
  ...
  """
  # Filter predictions DataFrame to only include rows with non-null scores
  df_predictions_filtered = df_predictions.dropna(subset=['score'])

  # Merges groundtruth and filtered predictions on uid, validates that uid is unique
