# Video Encoder code from source but very heavily modified [3]


# ResNet18 
# This code was originally based on [7]
...
class GlobalLayerNorm(nn.Module):
    ...
    def forward(self, y):
        ...
        gLN_y = self.gamma * (y - mean) / torch.sqrt(var + 1e-8) + self.beta
        return gLN_y
...
class visualFrontend(nn.Module):
    ...
    def forward(self, inputBatch):
        ...
        batch_size, channels, depth, height, width = batch.size()
        batch = batch.view(batch_size * depth, channels, height, width)
        outputBatch = self.resnet(batch)
        outputBatch = outputBatch.view(batch_size, depth, -1)
        return outputBatch

class NonTemporalVisualFrontend(nn.Module):
    def __init__(self):
        super(NonTemporalVisualFrontend, self).__init__()
        ...
        self.resnet = ResNet()  # Assuming ResNet is defined elsewhere to handle 2D inputs
...
class NonTemporalDSConv1d(nn.Module):
    def __init__(self):
        super(NonTemporalDSConv1d, self).__init__()
        self.net = nn.Sequential(
            nn.ReLU(),
            nn.BatchNorm1d(512),
            nn.Conv1d(512, 512, kernel_size=3, stride=1, padding=1, dilation=1, groups=512, bias=False),
            nn.PReLU(),
            nn.BatchNorm1d(512),
...
class NonTemporalVisualTCN(nn.Module):
    def __init__(self):
        super(NonTemporalVisualTCN, self).__init__()
        stacks = []        
        for _ in range(5):
            stacks.append(NonTemporalDSConv1d())
...
class NonTemporalVisualConv1D(nn.Module):
    def __init__(self):
        super(NonTemporalVisualConv1D, self).__init__()
        self.net = nn.Sequential(
