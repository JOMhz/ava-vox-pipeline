...
def load_train_csv(filename, column_names):
    """Loads CSV from the filename using given column names.
    Adds uid column.
    Args:
        filename: Path to the CSV file to load.
    ...
    # Convert 'frame_timestamp' to integers
    # df['frame_timestamp'] = df['frame_timestamp'].apply(convert_timestamp)
...
def merge_groundtruth_and_predictions(df_groundtruth, df_predictions):
  ...
  if df_groundtruth["uid"].count() != df_predictions["uid"].count():
    print(df_groundtruth["uid"].count())
    print(df_predictions["uid"].count())
    # missing_uids = set(df_groundtruth['uid']) - set(df_predictions['uid'])
    # missing_data = df_groundtruth[df_groundtruth['uid'].isin(missing_uids)]
    # sample_data = missing_data.sample(n=1)  # Adjust the sample size as needed
    # df_groundtruth = df_groundtruth[df_groundtruth['uid'] != '1800.0:9Y_l9NsnYE0_1740_1800:54']
    # print(sample_data)
    # raise ValueError(
...
def run_evaluation_asd(predictions, groundtruth, train=False):
  ...
  if not train:
    ...
    df_groundtruth = load_train_csv(groundtruth, column_names=column_names)
...
def get_eval_score(cfg, preds):
    ...
    if eval_type == 'AVA_ASD':
        ...
        groundtruth = os.path.join(path_annts, 'ava_activespeaker_train_v1.0.csv')
        score = run_evaluation_asd(preds, groundtruth, train=True)
        str_score = f'{score*100:.2f}%'
